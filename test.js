...
Elm.Board.make = function (_elm) {
   ...
   var viewCellStyle = F2(function (width,b) {
      return $Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "flex-basis",_1: A2($Basics._op["++"],$Basics.toString(100 / width | 0),"%")}]));
   });
   var viewStyle = $Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "display",_1: "flex"},{ctor: "_Tuple2",_0: "flex-flow",_1: "row wrap"}]));
   ...
   var assocIn = F3(function (model,position,b) {
      var _p0 = A2($Array.get,position.x,model);
      if (_p0.ctor === "Nothing") {
            return model;
         } else {
            var updatedRow = A3($Array.set,position.y,b,_p0._0);
            return A3($Array.set,position.x,updatedRow,model);
         }
   });
   var getIn = F2(function (model,position) {
      var _p1 = A2($Array.get,position.x,model);
      if (_p1.ctor === "Nothing") {
            return $Maybe.Nothing;
         } else {
            return A2($Array.get,position.y,_p1._0);
         }
   });
   var updateIn = F3(function (model,position,f) {
      var _p2 = A2(getIn,model,position);
      if (_p2.ctor === "Nothing") {
            return model;
         } else {
            return A3(assocIn,model,position,f(_p2._0));
         }
   });
   var isDead = F2(function (model,position) {
      var _p3 = A2(getIn,model,position);
      if (_p3.ctor === "Nothing") {
            return false;
         } else {
            return $Basics.not(_p3._0);
         }
   });
   var isAlive = function (model) {    return function (_p4) {    return $Basics.not(A2(isDead,model,_p4));};};
   ...
   var Next = {ctor: "Next"};
   var board = $Array.fromList(_U.list([$Array.fromList(_U.list([false,false,false,false,false,false,false,false]))
                                       ,$Array.fromList(_U.list([false,false,false,false,false,false,false,false]))
                                       ,$Array.fromList(_U.list([false,false,false,false,false,false,false,false]))
                                       ,$Array.fromList(_U.list([false,false,true,true,true,true,false,false]))
                                       ,$Array.fromList(_U.list([false,false,false,false,false,false,false,false]))
                                       ,$Array.fromList(_U.list([false,false,false,false,false,false,false,false]))
                                       ,$Array.fromList(_U.list([false,false,false,false,false,false,false,false]))]));
   var loadBoard = function (n) {    var _p5 = n;return board;};
   var width = function (model) {    var _p6 = A2($Array.get,0,model);if (_p6.ctor === "Nothing") {    return 0;} else {    return $Array.length(_p6._0);}};
   ...
   var neighbors = F2(function (model,pos) {
      ...
      function (_p7) {
         var _p8 = _p7;
         return A2(position,_p8._0,_p8._1);
      },
   ...
   var nextCellState = F2(function (model,position) {
      ...
      var _p9 = A2(getIn,model,position);
      if (_p9.ctor === "Nothing") {
            return false;
         } else {
            return A3(rules,_p9._0,deadCellCount,aliveCellCount);
         }
   });
   var positions = function (model) {
      var h = height(model);
      var w = width(model);
      return A2($List.concatMap,function (i) {    return A2($List.map,position(i),A2(range,0,w));},A2(range,0,h));
   };
   ...
   var update = F2(function (action,model) {
      ...
      {case "Next": return next(model);
         case "LoadBoard": return loadBoard(_p10._0);
         default: return A3(updateIn,model,_p10._0,$Basics.not);}
   });
   ...
   var view = F2(function (address,model) {
      ...
      _U.list([css("game.css")
              ,css("//maxcdn.bootstrapcdn.com/bootstrap/3.3.0/css/bootstrap.min.css")
              ,A2($Html.div,
              _U.list([$Html$Attributes.$class("board-container")]),
              A2($List.indexedMap,F2(function (k,b) {    return A4(viewCell,address,A2(intToPosition,k,w),w,b);}),$Array.toList(concat(model))))
              ,A2($Html.div,
              _U.list([$Html$Attributes.$class("button-container")]),
              _U.list([A2($Html.div,
              _U.list([$Html$Attributes.$class("btn btn-default"),A2($Html$Events.onClick,address,Next)]),
...
Elm.Main.make = function (_elm) {
   ...
   var _op = {};
   var main = $StartApp$Simple.start({model: $Board.loadBoard(0),update: $Board.update,view: $Board.view});
